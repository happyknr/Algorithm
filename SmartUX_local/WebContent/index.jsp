...
//	/app/apache-tomcat-8.0.28/svr_QATESTtx-web1/webapps/SmartUX/logs/com_skt_skaf_l001mtm091/BAT
	...
	/* ArrayList<String> arrPackageList = null;
	ArrayList<HashMap<String,String>> arrTestNameList = new ArrayList<HashMap<String,String>>();
	ArrayList<HashMap<String,String>> arrLogFileList = new ArrayList<HashMap<String,String>>();
	HashMap<String, String> mapTestNameList = null;
	HashMap<String, String> mapLogFileList = null; */
	...
	if(file.exists())
	{
		...
		//arrPackageList = new ArrayList<>(Arrays.asList(file.list()));
		for(int i = 0 ; i < packageList.length; i++)
		{
			...
			if(file2.exists())
			{
				...
				for(int j = 0 ; j < testNameList.length; j++)
				{
					...
					//mapTestNameList = new HashMap<String, String>();
					//System.out.println(">> " + testNameList[j]);
					if(file3.exists())
					{
						logFileList = file3.list();
						//logFileList = sortFileList(logFileList, COMPARETYPE_DATE);
						for(int z = 0; z < logFileList.length; z++)
						{
							//mapLogFileList = new HashMap<String, String>();
							//mapLogFileList.put(testNameList[j], logFileList[z]);
	...
	/* 상위 디렉토리명 넘기기 */
	filetest = new File(logFilePath+"\\");
	if(filetest.exists())
	{
		String[] listtest = filetest.list();
		System.out.println("-----------------------");
...
<%
	...
	{
%>
		<option value="<%=arrTestNameList.get(i)%>"><%=i + arrTestNameList.get(i)%></option>
<%
	}
%>
	</select>
